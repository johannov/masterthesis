...
total_pop <-  sum(df$SUM_pop_de)

df$weight <- df$SUM_pop_de / total_pop

df <- drop_na(df, any_of(c("wind_speed", "cloud_cover_total", "temperature_air_mean_200",
                         "celc", "hdd16")))
...
data_comp <- merge(data, weather_statemean, by = "date")
...
lm_pop <- lm(demand ~ hdd16 + dplyr::lag(hdd16) + wind_speed + cloud_cover_total, data = demdata)
summary(lm_pop)
