...
search_words = c("energiekrise", "erdgas", "nord stream")
all = gtrends(keyword = search_words, geo="DE", time="2018-01-01 2023-02-27")
tail(all$interest_over_time)

date_intervals = c("2018-01-01")
date=as.Date("2018-01-01")
while (date <= as.Date("2023-02-27")-7){
  date = as.Date(date + 7)
...
joined = inner_join(weekly_filled, daily_google, by=c("date", "keyword")) %>%
  ...
  dplyr::select(c("date","keyword","daily_hit","weekly_hit"))

#Adjust daily hits by monthly score
joined$adjusted = as.numeric(joined$weekly_hit)
joined$adjusted = ifelse(is.na(joined$adjusted), 0, joined$adjusted)
joined$final = joined$daily_hit * joined$adjusted/100
...
google_trend_indicator = joined %>% 
  group_by(date) %>% 
  summarise(hits = sum(final))

write.csv(google_trend_indicator, "data/google_trend_indicator.csv")
