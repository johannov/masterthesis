...
colnames(data) <- c("date", "hdd", "demand", "eex", "hddth", "hdd16")
...
demdata <- data %>% 
  group_by(week) %>% 
  mutate(demand = demand - mean(demand), hdd = hdd - mean(hdd), hddth = hddth - mean(hddth), hdd16 = hdd16 - mean(hdd16))
...
# ARDL model demand
model <- auto_ardl(demand ~ hdd16 + eex, data = demdata, max_order = 5, selection = "AIC")$best_model
summary(model)
multipliers(model)
multipliers(model, type = "sr")

# Simple linear regression
lm <- lm(demand ~ hdd16 + dplyr::lag(hdd16) + eex, data = demdata)
